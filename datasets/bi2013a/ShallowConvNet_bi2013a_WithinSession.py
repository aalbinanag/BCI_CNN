...
model = KerasClassifier(build_fn=lambda: ShallowConvNet(nb_classes, channels, samples, loss=loss, opt=opt, met=met), 
                        epochs=epochs, 
...
# Create pipeline
pipeline = Pipeline([('ShallowConvNet', model)])                    
...
results = evaluation.process({"ShallowConvNet": pipeline})
