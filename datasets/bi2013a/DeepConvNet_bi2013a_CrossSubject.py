...
model = KerasClassifier(build_fn=lambda: DeepConvNet(nb_classes, channels, samples, loss=loss, opt=opt, met=met), 
                        epochs=epochs, 
...
# Create pipeline
pipeline = Pipeline([('DeepConvNet', model)])                    
...
results = evaluation.process({"DeepConvNet": pipeline})
